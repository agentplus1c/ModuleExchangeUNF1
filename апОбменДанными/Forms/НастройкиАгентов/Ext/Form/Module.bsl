...
Процедура ЗаполнитьНастройкиКомпоновщика(СхемаКомпоновки, НастройкиКомпоновки)
	...
	КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиКомпоновки); // ВыгружаемыеДанныеПоУмолчанию = НастройкиКомпоновки; sk_191128 Изменено в рамках (MOD-971)

КонецПроцедуры

// sk_191128 Процедура исключена в рамках (MOD-971)
//&НаКлиенте
...
Процедура ОбновитьВыгружаемыеДанныеПоТекущемуАгенту(СтрокаТ = Неопределено)
	...
	// Выполняем загрузку пользовательских настроек
	...
	//	КонецЦикла;
	//КонецЕсли;
	текНастройкиАгента = СтрокаТ.ВыгружаемыеОбъекты; 
	Если текНастройкиАгента <> Неопределено Тогда 
		КомпоновщикНастроек.ЗагрузитьПользовательскиеНастройки(СтрокаТ.ВыгружаемыеОбъекты);
	Иначе
...
Функция ПолучитьИлиИнициализироватьНастройкуПоУмолчанию()
	...
	Если ТипЗнч(стррНастройки) <> Тип("Структура") Тогда // настройка "по умолчанию" еще ни разу не сохранялась
		...
		// ВыгужаемыеДанныеЗаполнитьЛевыеЗначения_OLD(); sk_191128 Процедура исключена в рамках (MOD-971) 		
