...
Функция ПроверитьКаталогДанных()
	...
	//(( sk_191125 Функция устарела. Следует использовать СтроковыеФункцииКлиентСервер.ПараметрыИзСтроки(). В рамках (MOD-970)  
	//стррПараметры = СтроковыеФункцииКлиентСервер.ПолучитьПараметрыИзСтроки(СтрокаСоединенияИнформационнойБазы());
	текОбъект = РеквизитФормыВЗначение("Объект");
	Если текОбъект.ПоколениеКонфигурацииСервер(">=УНФ_1.6.19.128", СтррКонтекст) Тогда 
