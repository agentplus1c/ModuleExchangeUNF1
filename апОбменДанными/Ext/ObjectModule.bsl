...
Процедура КонтекстФормыИнициализировать(СтррКонтекст, ПараметрыФормы, СтрОбязательныеСвойства = Неопределено) Экспорт
	...
	Если СтррКонтекст.Свойство("Конфигурация") Тогда
		...
		//(( sk_191125 Добавлено для сравнения версий на сервере в рамках (MOD-970)
...
// sk_191125 Добавлена в рамках (MOD-970)
//
...
Функция ПолучитьНастройкиАгента(Агент)
	...
	//(( sk_191128 Добавлено в рамаках (MOD-971)
	// Получаем основные настройки отбора на основе пользовательских настроек
	текНастройки = ПолучитьНастройкиОтбораАгентаПоПользовательскойНастройке(Результат.ВыгружаемыеОбъекты);
	Результат.ВыгружаемыеОбъекты = текНастройки;
	//)) sk_191128
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	//((vd_191014 Ошибка свойства "Назначение" при добавлении. Предположительно, передавать свойство не обязательно.
	//в 1.6.17 - ошибка
	Если ПоколениеКонфигурации("<УТ_1.6.17") Тогда
...
Функция ЗаписатьДанныеКонтактныхЛицАгентПлюс(ТекКонтрагент, ОбновитьДанные = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		...
		Иначе
			ТелефонПредставление = Нстр("ru = '8 (800) 333-6-321 или +7 (495) 662-59-01'; uk = '8 (800) 333-6-321 или +7 (495) 662-59-01'");
		КонецЕсли;
...
Функция ЭлементОтбораПоЛевомуЗначению(стрЛевоеЗначение, стрНазванияДополнительныхЭлементов = Неопределено )
	...
	//(( sk_191128 Изменено в рамках (MOD-971)
...
// sk_191128 Добавлена в рамках (MOD-970)
...
Функция ПолучитьКомпоновщикНастроекАгентаДляВыгрузки()
	...
	текСхемаКомпоновки = ПолучитьМакетОбработки(стрИмяМакета);
	Если текСхемаКомпоновки = Неопределено Тогда
		ВызватьИсключение(НСтр("ru = 'Функция ПолучитьКомпоновщикНастроекАгентаДляВыгрузки() - не удалось получить имя макета = '; uk = 'Функція ПолучитьКомпоновщикНастроекАгентаДляВыгрузки() - не вдалося отримати ім'я макета ='") + стрИмяМакета);
	КонецЕсли;
...
Функция ПолучитьНастройкиОтбораАгентаПоПользовательскойНастройке(ПользовательскиеНастройки)
	...
	Если ТипЗнч(ПользовательскиеНастройки) <> Тип("ПользовательскиеНастройкиКомпоновкиДанных") Тогда
		Текст = НСтр("ru='Агент: ""%1"", не заполнены ""Выгружаемые данные""! Проверьте ""Выгружаемые данные"" в настройках агента.';uk='Агент: ""%1"", незаповнені ""Вивантажувані дані""! Перевірте ""Вивантажувані дані"" в налаштуваннях агента.'");
		Текст = СтрШаблон_(Текст, ВыбАгент);
		ВызватьИсключение(Текст);
	КонецЕсли;
...
// sk_191114 Изменена в рамках (MOD-825)
// Выполняет выгрузку справочника "Номенклатура"
//
// Параметры:
//  ДокОбмена  - ЗаписьXML - запись xml 
...
Процедура ВыгрузитьНоменклатуру(ДокОбмена, МенеджерВТ)
	...
	текКоличествоРеквизитовЦен = 10;
	текКоличествоРеквизитовОстатков = 5;
	текВсегоВидовЦен = мВидыЦен.Количество();
	текВсегоСкладов  = мСклады.Количество();
	
	текСтрокВТабличнойЧасти = ПолучитьКоличествоСтрокТЧНоменклатуры(текВсегоСкладов, текКоличествоРеквизитовОстатков);
	Если текСтрокВТабличнойЧасти = 0 Тогда
		стррТЧОстатки = Неопределено;
	Иначе
		стррТЧОстатки = Новый Структура("GUID, Comment, КоличествоСтрок, ПолеВыгрузки, КоличествоРеквизитов, ИмяПоля, Номенклатура, ВыборкаЭлементов");
		стррТЧОстатки.GUID	   		   	   = "af0a6972-4bca-4652-a3cf-8ebc1ed1ee0d";
		стррТЧОстатки.Comment	   	   	   = "Остатки";	
		стррТЧОстатки.КоличествоСтрок  	   = текСтрокВТабличнойЧасти;
		стррТЧОстатки.ПолеВыгрузки 	   	   = "A06";
		стррТЧОстатки.КоличествоРеквизитов = текКоличествоРеквизитовОстатков;
		стррТЧОстатки.ИмяПоля 		   	   = "Остаток";
	КонецЕсли;
	
	текСтрокВТабличнойЧасти = ПолучитьКоличествоСтрокТЧНоменклатуры(текВсегоВидовЦен, текКоличествоРеквизитовЦен);
	Если текСтрокВТабличнойЧасти = 0 Тогда
		стррТЧЦены = Неопределено;
	Иначе
		стррТЧЦены = Новый Структура("GUID, Comment, КоличествоСтрок, ПолеВыгрузки, КоличествоРеквизитов, ИмяПоля, Номенклатура, ВыборкаЭлементов");
		стррТЧЦены.GUID	   		   	  	= "7f72aed2-94f7-4165-b112-c7a69fea70cc";
		стррТЧЦены.Comment	   	   		= "Цены";
		стррТЧЦены.КоличествоСтрок 		= текСтрокВТабличнойЧасти;
		стррТЧЦены.ПолеВыгрузки 	  	= "A011";
		стррТЧЦены.КоличествоРеквизитов = текКоличествоРеквизитовЦен;
		стррТЧЦены.ИмяПоля 		   		= "Цена";
	КонецЕсли;
	...
	флВключатьКодВНаименование = ВключатьКодыНоменклатурыВНаименование И ЗначениеЗаполнено(РежимВыгрузкиКодовНоменклатуры);
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		текСчЭлементов = текСчЭлементов + 1;
		...
		Если флВключатьКодВНаименование Тогда
		...
		Если ПоколениеКонфигурации(">=УНФ_1.6.17.128") Тогда
			...
			Если ЗначениеЗаполнено(ВыборкаНоменклатура.СтавкаНДС) Тогда
				текСтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ВыборкаНоменклатура.СтавкаНДС);
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(текСтавкаНДС) Тогда
				текСтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
			КонецЕсли;
		Иначе
			текСтавкаНДС = ВыборкаНоменклатура.СтавкаНДС;
		КонецЕсли;
		
		стррПоля.A04 = ПолучитьСтавкуНДСЧислом(текСтавкаНДС);
		стррПоля.A06 = текБазоваяЕдиницаИзмерения; // В реквизите A06 передается базовая единица измерения, относительно которой пересчитываются другие единицы
		стррПоля.A08 = ВыборкаНоменклатура.УчетПоХарактеристикам;
		стррПоля.A011 = ВыборкаОстатков.Остаток;
		стррПоля.A013 = текБазоваяЕдиницаИзмерения; // В реквизите A013 указывается единица измерения, в которой отображаются остатки товаров в форме подбора в мобильном устройстве
		...
		стррПоля.A042 = СокрЛП(ВыборкаНоменклатура.Комментарий);
		стррПоля.A043 = ВыборкаНоменклатура.ФайлКартинки;
		стррПоля.A044 = текСчЭлементов;
		стррПоля.A045 = ""; // Алкоголь
		стррПоля.A046 = ""; // Дата розлива
		стррПоля.A048 = ВыборкаНоменклатура.ЦеноваяГруппа;
		стррПоля.A050 = текБазоваяЕдиницаИзмерения; // В реквизите A050 передаем единицу цены. В эту единицу пересчитывается цена товара (относительно базовой) в форме подбора в МУ 
		...
		Пока ДетальныеЦены.Следующий() Цикл
			стррПоля.Вставить("A02" + Строка(текСч), ДетальныеЦены.Цена);	
			Если текСч = 9 Тогда
				Прервать;
			Иначе
				текСч = текСч + 1;
		...
		Пока ДетальныеОстатки.Следующий() Цикл
			стррПоля.Вставить("A03" + Строка(текСч), ДетальныеОстатки.Остаток);	
			Если текСч = 4 Тогда
				Прервать;
			Иначе
				текСч = текСч + 1;
		...
		ГруппировкиНоменклатурыМассивы(ВыборкаНоменклатура, мГруппыНоменклатуры);
	...
	ВыгрузитьГруппировкиНоменклатуры(ДокОбмена, мГруппыНоменклатуры);
...
// sk_191114 Изменена в рамках (MOD-825)
// Выполняет выгрузку справочника "ХарактеристикиНоменклатуры"
//
// Параметры:
//  ДокОбмена  - ЗаписьXML - запись xml 
//	МенеджерВТ - МенеджерВременнытТаблиц - менеджер ВТ
...
Процедура ВыгрузитьХарактеристикиНоменклатуры(ДокОбмена, МенеджерВТ)
	...
	ВыборкаНоменклатура		   = ДанныеПоХарактеристикаНоменклатуры(МенеджерВТ).Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	ВыборкаНоменклатураОстаток = ДанныеПоОстаткамХарактеристикаНоменклатуры(МенеджерВТ).Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	ВыборкаНоменклатураЦена    = ДанныеПоЦенамХарактеристикаНоменклатуры(МенеджерВТ).Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	// Номенклатура
	Пока ВыборкаНоменклатура.Следующий() Цикл
		ВыборкаНоменклатураОстаток.Следующий();
		ВыборкаНоменклатураЦена.Следующий();
		
		ВыборкаХарактеристика 		 = ВыборкаНоменклатура.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		ВыборкаХарактеристикаОстаток = ВыборкаНоменклатураОстаток.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		ВыборкаХарактеристикаЦена 	 = ВыборкаНоменклатураЦена.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		// Характеристика
		Пока ВыборкаХарактеристика.Следующий() Цикл
			ВыборкаХарактеристикаОстаток.Следующий();
			ВыборкаХарактеристикаЦена.Следующий();
			
			ДетальныеХарактеристика = ВыборкаХарактеристика.Выбрать(); 
			ДетальныеОстатки 	    = ВыборкаХарактеристикаОстаток.Выбрать();
			ДетальныеЦены    		= ВыборкаХарактеристикаЦена.Выбрать();
			// Детализация
			Пока ДетальныеХарактеристика.Следующий() Цикл 
				стррПоискХарактеристики.Номенклатура = ДетальныеХарактеристика.Номенклатура;
				стррПоискХарактеристики.Ссылка  	 = ДетальныеХарактеристика.Характеристика;
				
				текИдХарактеристики = СТЗИдентификаторЭлементаДляМТ(стррПоискХарактеристики, "СпрХарактеристикиНоменклатурыСлужебный");
				
				стррПоля.GUID = текИдХарактеристики;
				стррПоля.Name = ДетальныеХарактеристика.Наименование;
				стррПоля.A02  = ДетальныеХарактеристика.Номенклатура;
				стррПоля.A04  = ВыборкаХарактеристикаОстаток.Остаток;
				стррПоля.A037 = ВыборкаХарактеристикаОстаток.Остаток > 0;
				стррПоля.A039 = ВыборкаХарактеристикаОстаток.ОстатокНаБорту;
				...
				Если флВыгружатьТабличныеЧасти Тогда
					...
					Если стррТЧОстатки <> Неопределено Тогда
						ЗаписатьНачалоТабличнойЧасти(ДокОбмена, стррТЧОстатки);
						стррТЧОстатки.Номенклатура = текИдХарактеристики;
						стррТЧОстатки.ВыборкаЭлементов = ДетальныеОстатки;
						ВыгрузитьПоляТабличнойЧастиНоменклатуры(ДокОбмена, стррТЧОстатки);
						ЗаписатьОкончаниеТабличнойЧасти(ДокОбмена);
					КонецЕсли;
					...
					Если стррТЧЦены <> Неопределено Тогда
						ЗаписатьНачалоТабличнойЧасти(ДокОбмена, стррТЧЦены);
						стррТЧЦены.Номенклатура = текИдХарактеристики;
						стррТЧЦены.ВыборкаЭлементов = ДетальныеЦены;
						ВыгрузитьПоляТабличнойЧастиНоменклатуры(ДокОбмена, стррТЧЦены);
						ЗаписатьОкончаниеТабличнойЧасти(ДокОбмена);
					КонецЕсли;
...
// sk_191114 Добавлена в рамках (MOD-825)
// Выполняет заполнение элементов группировки справочника "Номенклатура"
//
// Параметры:
//  Выборка  			- ВыборкаИзРезультатаЗапроса - выборка из запроса
//  мГруппыНоменклатуры - Массив - группы номенклатуры
//
Процедура ГруппировкиНоменклатурыМассивы(Выборка, мГруппыНоменклатуры)

	// Группа
	Если Выборка.Родитель <> Справочники.Номенклатура.ПустаяСсылка() 
		И Выборка.Родитель <> Неопределено Тогда
		Если мГруппыНоменклатуры.Найти(Выборка.Родитель) = Неопределено Тогда
			мГруппыНоменклатуры.Добавить(Выборка.Родитель);
		КонецЕсли;
	КонецЕсли;
...
// sk_191114 Изменена в рамках (MOD-825)
//
// Выполняет выгрузку группировки справочника "Номенклатура"
//
// Параметры:
//	ДокОбмена  			- ЗаписьXML - запись xml 
//	мГруппыНоменклатуры - Массив - список групп номенклатуры
//
// ВыгрузитьГруппировкиНоменклатуры(ДокОбмена, МенеджерВТ)
Процедура ВыгрузитьГруппировкиНоменклатуры(ДокОбмена, мГруппыНоменклатуры)
	...
	//ДокОбмена.ЗаписатьКонецЭлемента();
	//ДокОбмена.ЗаписатьКонецЭлемента();
	ВыгрузитьГруппировкиНоменклатурыПоРодителю(ДокОбмена, мГруппыНоменклатуры);
	//)) sk_191114
...
// sk_191114 Изменена в рамках (MOD-825)
// Выполняет выгрузка группировки справочника "Номенклатура" по родителю
//
// Параметры:
//	ДокОбмена  			- ЗаписьXML - запись xml 
//	мГруппыНоменклатуры - Массив - список групп номенклатуры
//
Процедура ВыгрузитьГруппировкиНоменклатурыПоРодителю(ДокОбмена, Знач мГруппыНоменклатуры)

	Если НЕ (ТипЗнч(мГруппыНоменклатуры) = Тип("Массив") И мГруппыНоменклатуры.Количество() > 0) Тогда
		Возврат;
	КонецЕсли;
	
	тзСписокГрупп = Новый ТаблицаЗначений;
	тзСписокГрупп.Колонки.Добавить("Ссылка");
	тзСписокГрупп.Колонки.Добавить("Наименование");
	тзСписокГрупп.Колонки.Добавить("Родитель"); 
	
	ЗаполнитьРодителейСправочникаРекурсивно("Номенклатура", тзСписокГрупп, мГруппыНоменклатуры);
	
	тзСписокГрупп.Сортировать("Наименование");
	
	// Объявление группировки по родителю
	ДокОбмена.ЗаписатьНачалоЭлемента("GROUP");
	...
	Для Каждого стзГруппа Из тзСписокГрупп Цикл
		стррПоля.GUID = стзГруппа.Ссылка.УникальныйИдентификатор();
		стррПоля.Name = стзГруппа.Наименование;
		стррПоля.ParId = стзГруппа.Родитель;							
		ЗаписатьЭлементВДокументОбмена(ДокОбмена, стррПоля);	                  					
	КонецЦикла;
...
// sk_191114 Добавлена в рамках (MOD-825)
// Выполняет заполнение родителей справочника рекурсивно
//
// Параметры:
//  Справочник 	  	 - Текст - наименование справочника
//  тзСписокГрупп    - Таблица значений - список групп
//  мСписокРодителей - Массив - список родителей
//
Процедура ЗаполнитьРодителейСправочникаРекурсивно(Справочник, тзСписокГрупп, мСписокРодителей)

	// 1. Проверяем список родителей,
	// рекурсия завершается при отсутствии родителей, если дошли до самого верхнего уровня иерархии
	Если мСписокРодителей.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
					...
					|ГДЕ
					|	СправочникГруппировки.Ссылка В(&СписокРодителей)";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "[СправочникГруппировки]", Справочник);
	Запрос.УстановитьПараметр("СписокРодителей", мСписокРодителей);
	
	тзВыборкаЭлементов = Запрос.Выполнить().Выгрузить();

	// 3. Выполняем очистку списка родителей	
	мСписокРодителей.Очистить();
	
	// 4. Выполняем перебор элементов,
	// новые элементы добавляем в тзГруппы, 
	// новых родителей добавляем в мСписокРодителей  
	Для Каждого стзЭлемент Из тзВыборкаЭлементов Цикл
		Если тзСписокГрупп.Найти(стзЭлемент.Ссылка, "Ссылка") = Неопределено Тогда
			стзГруппа = тзСписокГрупп.Добавить();
			стзГруппа.Ссылка	   = стзЭлемент.Ссылка;
			стзГруппа.Наименование = стзЭлемент.Наименование;
			стзГруппа.Родитель     = стзЭлемент.Родитель; 
		КонецЕсли;
		
		Если ЗначениеЗаполнено(стзЭлемент.Родитель) И мСписокРодителей.Найти(стзЭлемент.Родитель) = Неопределено И тзСписокГрупп.Найти(стзЭлемент.Родитель, "Ссылка") = Неопределено Тогда
			мСписокРодителей.Добавить(стзЭлемент.Родитель);
		КонецЕсли;
	КонецЦикла;
	
	ЗаполнитьРодителейСправочникаРекурсивно(Справочник, тзСписокГрупп, мСписокРодителей);

КонецПроцедуры

// sk_191114 Процедура исключена в рамках (MOD-825)
//
Процедура РекурсивноЗаписатьГруппуСправочника_OLD(ДокОбмена, Строки)
	...
	Для каждого ТекГруппа Из Строки Цикл
		...
		ЗаписатьЭлементВДокументОбмена(ДокОбмена, стррПоля);
		РекурсивноЗаписатьГруппуСправочника_OLD(ДокОбмена, ТекГруппа.Строки);
...
// sk_191114 Добавлена в рамках (MOD-825)
// Возвращает количество табличных частей
//
// Параметры:
//	КоличествоЕдиниц 	- Число - общее количество элементов
//	КоличествоАтрибутов - Число - количество элементов в табличной части 
//
// Возвращаемое значение:
//  текКолТЧ - Число - количество табличных частей
//
Функция ПолучитьКоличествоСтрокТЧНоменклатуры(КоличествоЕдиниц, КоличествоАтрибутов)
	...
	текСтрокВТЧ = КоличествоЕдиниц / КоличествоАтрибутов;
	
	текКолТЧ = Цел(текСтрокВТЧ);
...
Функция СкладыВыгрузки(МенеджерВТ, ВернутьМассив)
	...
	//(( sk_191114 Исключено АВТОУПОРЯДОЧЕВАНИЕ, т.к. некорректно выполяется сортировка по ссылке в рамках (MOD-939) 
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      ...
	                      |	СпрСклады.Наименование КАК НаименованиеВТ
	                      |ИЗ
	                      |	Справочник.СтруктурныеЕдиницы КАК СпрСклады
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Склады КАК ВТ_Склады
	                      |		ПО СпрСклады.Ссылка = ВТ_Склады.Ссылка
	                      |ГДЕ
	                      |	СпрСклады.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	                      |
	                      |УПОРЯДОЧИТЬ ПО
...
// sk_191114 Исключена в рамках (MOD-825) 
// ВыгрузитьПоляВЭлемент()
//
Процедура ВыгрузитьПоляВЭлемент_OLD(ПоляЭлемента, ВыборкаЭлементов, КоличествоРеквизитов, ИмяПоля, ПолеВыгрузки)
...
Процедура ВыгрузитьПоляТабличнойЧастиНоменклатуры(ДокОбмена, стррПараметры)
	...
	текМаксЗначениеСчетчика = стррПараметры.КоличествоРеквизитов; // МаксЗначениеСчетчика = стррПараметры.КоличествоРеквизитов - 1; sk_191114
	стрИмяПоля = стррПараметры.ИмяПоля;	
	стрПолеВыгрузки = стррПараметры.ПолеВыгрузки;
	
	стррПоля = Новый Структура("GUID, CtlgId, " + стрПолеВыгрузки);
	
	Для текСчСтрок = 1 По стррПараметры.КоличествоСтрок Цикл
		
		стррПоля.GUID = Новый УникальныйИдентификатор();
		стррПоля.CtlgId = стррПараметры.Номенклатура;
		стррПоля[стрПолеВыгрузки] = текСчСтрок;
		текСч = 0;
		Пока ВыборкаЭлементов.Следующий() Цикл
			текСч = текСч + 1;
			стррПоля.Вставить("A0" + Строка(текСч), ВыборкаЭлементов[стрИмяПоля]);
			Если текСч = текМаксЗначениеСчетчика Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
		ЗаписатьЭлементВДокументОбмена(ДокОбмена, стррПоля);
...
Функция ЗапросПоНоменклатуре_OLD(МенеджерВТ, ДоступныеВидыЦен)
	...
	Запрос = Новый Запрос(ТекстЗапросаВыгрузкиНоменклатуры_OLD());
...
//
// Возвращаемое значение: 
//  Результат - ВыборкаИзРезультатаЗапроса  
//
Функция ДанныеПоНоменклатуре(МенеджерВТ)
	
	//В справочнике Номенклатура
	// 1.6.15.39   СтавкаНДС
	// 1.6.17.123  ВидСтавкиНДС	
	// 1.6.17.128  ВидСтавкиНДС
	Если ПоколениеКонфигурации(">=УНФ_1.6.17") Тогда
		стрСтавкаНДС = "СпрНоменклатура.ВидСтавкиНДС";
	Иначе
		стрСтавкаНДС = "СпрНоменклатура.СтавкаНДС";	
	КонецЕсли;
	
	//В справочнике Номенклатура
	// 1.6.9.54  - Нет реквизита Вес
	// 1.6.10.38 - Есть реквизит Вес
	Если ПоколениеКонфигурации(">=УНФ_1.6.10.38") Тогда 
		стрВесовой = "ВЫБОР КОГДА СпрНоменклатура.Вес > 0 ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ КОНЕЦ";
	Иначе
		стрВесовой = "ЛОЖЬ";
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СпрНоменклатура.ЕдиницаИзмерения КАК БазоваяЕдиницаИзмерения,
	               |	ВЫБОР
	               |		КОГДА СпрНоменклатура.ИспользоватьХарактеристики = ЛОЖЬ
	               |			ТОГДА ЛОЖЬ
	               |		ИНАЧЕ ИСТИНА
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СтавкаНДС", стрСтавкаНДС);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Весовой",   стрВесовой);
...
Функция ДанныеПоОстаткамНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				ТОГДА ЕСТЬNULL(ВТ_НоменклатураОстатки.Остаток, 0)
	               |			ИНАЧЕ 0
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтруктурныеЕдиницы КАК СпрСклады
...
// sk_191114 Добавлена в рамках (MOD-825)
// Возвращает запрос по ценам Номенклатуры
//
// Параметры:
//  МенеджерВТ - МенеджерВременнытТаблиц - менеджер ВТ
//
...
Функция ДанныеПоЦенамНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ПОМЕСТИТЬ ВТ_НоменклатураИВидыЦен
	               |ИЗ
	               |	ВТ_Номенклатура КАК ВТ_Номенклатура,
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_НоменклатураИВидыЦен.Номенклатура КАК Номенклатура,
	               |	ВТ_НоменклатураИВидыЦен.ВидЦен КАК ВидЦен,
	               |	ЕСТЬNULL(ВТ_НоменклатураЦены.Цена, 0) КАК Цена
	               |ИЗ
	               |	ВТ_НоменклатураИВидыЦен КАК ВТ_НоменклатураИВидыЦен
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_НоменклатураЦены КАК ВТ_НоменклатураЦены
	               |		ПО ВТ_НоменклатураИВидыЦен.Номенклатура = ВТ_НоменклатураЦены.Номенклатура
	               |			И ВТ_НоменклатураИВидыЦен.ВидЦен = ВТ_НоменклатураЦены.ВидЦен
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
	               |		ПО ВТ_НоменклатураИВидыЦен.Номенклатура = СпрНоменклатура.Ссылка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЦен КАК СпрВидыЦен
	               |		ПО ВТ_НоменклатураИВидыЦен.ВидЦен = СпрВидыЦен.Ссылка
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	СпрНоменклатура.Наименование,
	               |	СпрНоменклатура.Код,
	               ...
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_НоменклатураИВидыЦен
				   |;
				   |
				   |////////////////////////////////////////////////////////////////////////////////
...
// sk_191114 Добавлена в рамках (MOD-825)
// Возвращает запрос по остаткам ХарактеристикиНоменклатуры
//
// Параметры:
//  МенеджерВТ - МенеджерВременнытТаблиц - менеджер ВТ
//
...
//  Результат - ВыборкаИзРезультатаЗапроса 
//
Функция ДанныеПоХарактеристикаНоменклатуры(МенеджерВТ)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВТ_Характеристика.Номенклатура КАК Номенклатура,
	               |	ВТ_Характеристика.Характеристика КАК Характеристика,
				   |	СпрХарактеристика.НаименованиеДляПечати КАК Наименование
	               |ИЗ
	               |	ВТ_Характеристика КАК ВТ_Характеристика
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
	               |		ПО ВТ_Характеристика.Номенклатура = СпрНоменклатура.Ссылка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК СпрХарактеристика
	               |		ПО ВТ_Характеристика.Характеристика = СпрХарактеристика.Ссылка
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	СпрНоменклатура.Наименование,
	               |	СпрНоменклатура.Код,
...
//  Результат - ВыборкаИзРезультатаЗапроса  
//
Функция ДанныеПоОстаткамХарактеристикаНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВТ_Характеристика.Характеристика КАК Характеристика,
	               |	ВТ_Склады.Ссылка КАК Склад
	               |ПОМЕСТИТЬ ВТ_НоменклатураХарактеристикаИСклады
	               |ИЗ
	               |	ВТ_Характеристика КАК ВТ_Характеристика,
	               |	ВТ_Склады КАК ВТ_Склады
	               |
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_НоменклатураХарактеристикаИСклады.Номенклатура КАК Номенклатура,
	               |	ВТ_НоменклатураХарактеристикаИСклады.Характеристика КАК Характеристика,
	               |	ВТ_НоменклатураХарактеристикаИСклады.Склад КАК Склад,
	               |	СУММА(ЕСТЬNULL(ВТ_Остатки.Остаток, 0)) КАК Остаток,
	               |	СУММА(ВЫБОР
	               |			КОГДА ВТ_НоменклатураХарактеристикаИСклады.Склад = &МобильныйСклад
	               |				ТОГДА ЕСТЬNULL(ВТ_Остатки.Остаток, 0)
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК ОстатокНаБорту
	               |ПОМЕСТИТЬ ВТ_НоменклатураХарактеристикаОстатокГР
	               |ИЗ
	               |	ВТ_НоменклатураХарактеристикаИСклады КАК ВТ_НоменклатураХарактеристикаИСклады
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Остатки КАК ВТ_Остатки
	               |		ПО ВТ_НоменклатураХарактеристикаИСклады.Номенклатура = ВТ_Остатки.Номенклатура
	               |			И ВТ_НоменклатураХарактеристикаИСклады.Характеристика = ВТ_Остатки.Характеристика
	               |			И ВТ_НоменклатураХарактеристикаИСклады.Склад = ВТ_Остатки.Склад
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВТ_НоменклатураХарактеристикаИСклады.Номенклатура,
	               |	ВТ_НоменклатураХарактеристикаИСклады.Характеристика,
	               |	ВТ_НоменклатураХарактеристикаИСклады.Склад
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Номенклатура,
	               |	Характеристика,
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_НоменклатураХарактеристикаОстатокГР.Номенклатура КАК Номенклатура,
	               |	ВТ_НоменклатураХарактеристикаОстатокГР.Характеристика КАК Характеристика,
	               |	ВТ_НоменклатураХарактеристикаОстатокГР.Склад КАК Склад,
	               |	ВТ_НоменклатураХарактеристикаОстатокГР.Остаток КАК Остаток,
	               |	ВТ_НоменклатураХарактеристикаОстатокГР.ОстатокНаБорту КАК ОстатокНаБорту
	               |ИЗ
	               |	ВТ_НоменклатураХарактеристикаОстатокГР КАК ВТ_НоменклатураХарактеристикаОстатокГР
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
	               |		ПО ВТ_НоменклатураХарактеристикаОстатокГР.Номенклатура = СпрНоменклатура.Ссылка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК СпрХарактеристика
	               |		ПО ВТ_НоменклатураХарактеристикаОстатокГР.Характеристика = СпрХарактеристика.Ссылка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтруктурныеЕдиницы КАК СпрСклады
	               |		ПО ВТ_НоменклатураХарактеристикаОстатокГР.Склад = СпрСклады.Ссылка
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	СпрНоменклатура.Наименование,
	               |	СпрНоменклатура.Код,
	               ...
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_НоменклатураХарактеристикаИСклады
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_НоменклатураХарактеристикаОстатокГР
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
...
// sk_191114 Добавлена в рамках (MOD-825)
// Возвращает запрос по ценам ХарактеристикиНоменклатуры
//
// Параметры:
//  МенеджерВТ - МенеджерВременнытТаблиц - менеджер ВТ
//
...
//  Результат - ВыборкаИзРезультатаЗапроса  
//
Функция ДанныеПоЦенамХарактеристикаНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВТ_Характеристика.Характеристика КАК Характеристика,
	               |	ВТ_ВидыЦен.ВидЦен КАК ВидЦен
	               |ПОМЕСТИТЬ ВТ_НоменклатураХарактеристикаИВидЦен
	               |ИЗ
	               |	ВТ_Характеристика КАК ВТ_Характеристика,
	               |	ВТ_ВидыЦен КАК ВТ_ВидыЦен
	               |
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_НоменклатураХарактеристикаИВидЦен.Номенклатура КАК Номенклатура,
	               |	ВТ_НоменклатураХарактеристикаИВидЦен.Характеристика КАК Характеристика,
	               |	ВТ_НоменклатураХарактеристикаИВидЦен.ВидЦен КАК ВидЦен,
	               |	МАКСИМУМ(ЕСТЬNULL(ВТ_Цены.Цена, 0)) КАК Цена
	               |ПОМЕСТИТЬ ВТ_НоменклатураХарактеристикаЦенаГР
	               |ИЗ
	               |	ВТ_НоменклатураХарактеристикаИВидЦен КАК ВТ_НоменклатураХарактеристикаИВидЦен
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Цены КАК ВТ_Цены
	               |		ПО ВТ_НоменклатураХарактеристикаИВидЦен.Номенклатура = ВТ_Цены.Номенклатура
	               |			И ВТ_НоменклатураХарактеристикаИВидЦен.Характеристика = ВТ_Цены.Характеристика
	               |			И ВТ_НоменклатураХарактеристикаИВидЦен.ВидЦен = ВТ_Цены.ВидЦен
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВТ_НоменклатураХарактеристикаИВидЦен.Номенклатура,
	               |	ВТ_НоменклатураХарактеристикаИВидЦен.Характеристика,
	               |	ВТ_НоменклатураХарактеристикаИВидЦен.ВидЦен
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Номенклатура,
	               |	Характеристика,
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_НоменклатураХарактеристикаЦенаГР.Номенклатура КАК Номенклатура,
	               |	ВТ_НоменклатураХарактеристикаЦенаГР.Характеристика КАК Характеристика,
	               |	ВТ_НоменклатураХарактеристикаЦенаГР.ВидЦен КАК ВидЦен,
	               |	ВТ_НоменклатураХарактеристикаЦенаГР.Цена КАК Цена
	               |ИЗ
	               |	ВТ_НоменклатураХарактеристикаЦенаГР КАК ВТ_НоменклатураХарактеристикаЦенаГР
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
	               |		ПО ВТ_НоменклатураХарактеристикаЦенаГР.Номенклатура = СпрНоменклатура.Ссылка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК СпрХарактеристика
	               |		ПО ВТ_НоменклатураХарактеристикаЦенаГР.Характеристика = СпрХарактеристика.Ссылка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЦен КАК СпрВидыЦен
	               |		ПО ВТ_НоменклатураХарактеристикаЦенаГР.ВидЦен = СпрВидыЦен.Ссылка
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	СпрНоменклатура.Наименование,
	               |	СпрНоменклатура.Код,
	               ...
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_НоменклатураХарактеристикаИВидЦен
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_НоменклатураХарактеристикаЦенаГР
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_Цены
...
// ТекстЗапросаВыгрузкиНоменклатуры()
//
Функция ТекстЗапросаВыгрузкиНоменклатуры_OLD()
	
	//((vd_190405 У справочника номенклатура удален реквизит Справочник.Номенклатура.СтавкаНДС, вместо него добавлен ВидСтавкиНДС
	//  1.6.15.39  СтавкаНДС
	//	1.6.17.123	ВидСтавкиНДС	
	//	1.6.17.128  ВидСтавкиНДС
	//vd_191014 Если ПоколениеКонфигурации(">=УНФ_1.6.17.128") Тогда
	Если ПоколениеКонфигурации(">=УНФ_1.6.17") Тогда
...
Процедура МенеджерВТОстатки(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВТ_Ассортимент.Ссылка КАК Номенклатура,
	               |	ЕСТЬNULL(ЗапасыНаСкладах.Характеристика, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.Пустаяссылка)) КАК Характеристика,
	               |	ЕСТЬNULL(ЗапасыНаСкладах.СтруктурнаяЕдиница, ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)) КАК Склад,
	               |	ЕСТЬNULL(ВЫБОР
	               |			КОГДА &УчитыватьРезервы
	               |				ТОГДА ЗапасыНаСкладах.КоличествоОстаток - ЕСТЬNULL(РазмещениеЗаказов.КоличествоОстаток, 0)
	               |			ИНАЧЕ ЗапасыНаСкладах.КоличествоОстаток
	               |		КОНЕЦ, 0) КАК Остаток
	               |ПОМЕСТИТЬ ВТ_ОстаткиБезГруппировки
	               |ИЗ
	               |	ВТ_Ассортимент КАК ВТ_Ассортимент
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыНаСкладах.Остатки(, ) КАК ЗапасыНаСкладах
	               |		ПО ВТ_Ассортимент.Ссылка = ЗапасыНаСкладах.Номенклатура
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Склады КАК ВТ_Склады
	               |		ПО (ЗапасыНаСкладах.СтруктурнаяЕдиница = ВТ_Склады.Ссылка)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РазмещениеЗаказов.Остатки(, ) КАК РазмещениеЗаказов
	               |		ПО ВТ_Ассортимент.Ссылка = РазмещениеЗаказов.Номенклатура
	               |ГДЕ
	               |	ВЫБОР
	               |			КОГДА &ВыгружатьТоварыСНулевымОстатком
	               |				ТОГДА ИСТИНА
...
Процедура МенеджерВТЦеныНоменклатуры(МенеджерВТ)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	СпрВидыЦен.Ссылка КАК ВидЦен
	               |ПОМЕСТИТЬ ВТ_ВидыЦен
	               |ИЗ
	               |	Справочник.ВидыЦен КАК СпрВидыЦен
	               |ГДЕ
	               |	СпрВидыЦен.Ссылка В(&ВидыЦен)
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	ВидЦен
	               |;
	               ...
	               |	ВТ_НоменклатураХарактеристика.Характеристика КАК Характеристика,
	               |	ВТ_ВидыЦен.ВидЦен КАК ВидЦен
	               |ПОМЕСТИТЬ ВТ_НоменклатураХарактеристикаИВидыЦен
	               |ИЗ
	               |	ВТ_НоменклатураХарактеристика КАК ВТ_НоменклатураХарактеристика,
	               |	ВТ_ВидыЦен КАК ВТ_ВидыЦен
	               |
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_НоменклатураХарактеристикаИВидыЦен.Номенклатура КАК Номенклатура,
	               |	ВТ_НоменклатураХарактеристикаИВидыЦен.Характеристика КАК Характеристика,
	               |	ВТ_НоменклатураХарактеристикаИВидыЦен.ВидЦен КАК ВидЦен,
	               |	ВЫРАЗИТЬ(ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) КАК ЧИСЛО(15, 2)) КАК Цена
	               |ПОМЕСТИТЬ ВТ_Цены
	               |ИЗ
	               |	ВТ_НоменклатураХарактеристикаИВидыЦен КАК ВТ_НоменклатураХарактеристикаИВидыЦен
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&Дата, ) КАК ЦеныНоменклатуры
	               |		ПО ВТ_НоменклатураХарактеристикаИВидыЦен.Номенклатура = ЦеныНоменклатуры.Номенклатура
	               |			И ВТ_НоменклатураХарактеристикаИВидыЦен.Характеристика = ЦеныНоменклатуры.Характеристика
	               |			И ВТ_НоменклатураХарактеристикаИВидыЦен.ВидЦен = ЦеныНоменклатуры.ВидЦен
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Номенклатура,
	               |	Характеристика,
	               ...
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_НоменклатураХарактеристикаИВидыЦен";
...
Функция СформироватьАрхивСКартинками(тзКартинки)
	...
	Для Каждого СтрокаТ Из тзКартинки Цикл			
		...
		//(( sk_191125 Изменены процедуры общего модуля с ПрисоединенныеФайлы на РаботаСФайлами в рамках (MOD-952) 
		// УНФ 1.6.17.128 ПрисоединенныеФайлы
		// УНФ 1.6.19.128 РаботаСФайлами
		//
		...
		стрПолноеИмяФайла = КаталогКартинок + СтрокаТ.ИмяФайла;
		Если Не ФайлСуществует(стрПолноеИмяФайла) Тогда // Если файла в каталоге нет, записываем его из БД
			Если ПоколениеКонфигурации(">=УНФ_1.6.19.128") Тогда
				ОбщийМодульРаботысФайлами = Вычислить("РаботаСФайлами");
				Попытка
					ДанныеФайла = ОбщийМодульРаботысФайлами.ДвоичныеДанныеФайла(ФайлСсылка);
				Исключение
					Продолжить;
				КонецПопытки;
				...
				Иначе
					Продолжить;
				КонецЕсли;
				Картинка.Записать(стрПолноеИмяФайла);
			Иначе
				ОбщийМодульПрисоединенныеФайлы = Вычислить("ПрисоединенныеФайлы");
				стррДанные = ОбщийМодульПрисоединенныеФайлы.ПолучитьДанныеФайла(ФайлСсылка);
				ДвоичныеДанные = ПолучитьИзВременногоХранилища(стррДанные.СсылкаНаДвоичныеДанныеФайла);
				Если ДвоичныеДанные = Неопределено Тогда
					Продолжить;
				Иначе
					ДвоичныеДанные.Записать(стрПолноеИмяФайла);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		АрхивКартинок.Добавить(стрПолноеИмяФайла);
		//)) sk_191125
	КонецЦикла;	
...
Функция ЗагрузитьДокументЗаказ(ТекстXML, стррПараметры)
	...
	//(( sk_191203 Перезаполняем Вид цены из файла
	ОбъектДок.ВидЦен = ВидЦены; 
	//)) sk_191203
	
	//(( sk_191203 Заполняем налогооблажение НДС в рамках (MOD-839)
	ЗаполнитьНалогооблажениеНДСДокумента(ОбъектДок);
	//)) sk_191203

	// +АгентПлюс 2018-11-30 Виктор
	//ДозаполнитьПоУмолчанию
	Если гКонфигурация = "УНФ_UA" Или ПоколениеКонфигурации(">=УНФ_1.6.7") Тогда // Если гКонфигурация = "УНФ_UA" Тогда sk_191203 Изменено условие
	...
	Если ТекстXML.Имя = "TABLES" И ТекстXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		...
		Если НРег(СокрЛП(ТекстXML.ПолучитьАтрибут("GUID"))) = ИдТЧТовары Тогда
			...
			//(( sk_191203 Выполняем расчет строки табличной части
			...
			Если ПоколениеКонфигурации(">=УНФ_1.6.15.39") Тогда
				ОбщМодуль = Вычислить("НоменклатураВДокументахКлиентСервер");
			Иначе
				ОбщМодуль = Вычислить("РаботаСНоменклатуройКлиентСервер");
			КонецЕсли;
			ЗаполнитьСтавкуНДСТабЧасти(НоваяСтрока, ОбъектДок); 
			ОбщМодуль.РассчитатьСуммуВСтрокеТабличнойЧасти(ОбъектДок, НоваяСтрока, "Запасы");	
			//)) sk_191203
	...
	Если ТекстXML.Имя = "TABLES" И ТекстXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		...
		Если НРег(СокрЛП(ТекстXML.ПолучитьАтрибут("GUID"))) = ИдТЧТовары Тогда
			...
			Пока ТекстXML.Прочитать() Цикл
				...
				Конецесли;
				НоваяСтрокаТЧ.ЕдиницаИзмерения	   = ТекЕдиница;
				НоваяСтрокаТЧ.Цена 				   = Цена;
				НоваяСтрокаТЧ.ПроцентСкидкиНаценки = ПроцентРучнойСкидки;
				НоваяСтрокаТЧ.Сумма 		 	   = Сумма;
				НоваяСтрокаТЧ.Количество 		   = Количество;
				
				//(( sk_191203 Изменено заполнение ставки НДС в рамках (MOD-521)
				//Если СтавкаНДС > 0 Тогда
				...
				//КонецЕсли;
				НоваяСтрокаТЧ.СтавкаНДС = ПолучитьСтавкуНДС(НоваяСтрокаТЧ);
				//)) sk_191203
				...
				Иначе
					ОбщМодуль = Вычислить("РаботаСНоменклатуройКлиентСервер");
				КонецЕсли;
				ЗаполнитьСтавкуНДСТабЧасти(НоваяСтрокаТЧ, ОбъектДок); 
				ОбщМодуль.РассчитатьСуммуВСтрокеТабличнойЧасти(ОбъектДок, НоваяСтрокаТЧ, "Запасы");	
				//)) sk_191203
				//}}
...
// sk_191203 Добавлена в рамках (MOD-839)
// Выполняет заполнение типа налогооблажения продажи товаров и услуг налогом на добавленную стоимость
//
// Параметры:
...
Процедура ЗаполнитьНалогооблажениеНДСДокумента(ОбъектДок)
	...
	ОбъектДок.НалогообложениеНДС = УправлениеНебольшойФирмойСервер.НалогообложениеНДС(ОбъектДок.Организация,, ОбъектДок.Дата);
 
КонецПроцедуры

// sk_191203 Добавлена в рамках
// Выполняет заполнение ставки НДС по строке табличности части
//
// Параметры:
//	СтрокаТабличнойЧасти - ДокументТабличнаяЧастьСтрока - строка табличной части документа 
//	ОбъектДок            - ДокументОбъект - документ  
Процедура ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ОбъектДок)
	
	текСтавкаНДС = Неопределено;
	
	Если ОбъектДок.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС") Тогда
		Если ПоколениеКонфигурации(">=УНФ_1.6.17") Тогда
			текСтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(СтрокаТабличнойЧасти.Номенклатура.ВидСтавкиНДС, ТекущаяДатаСеанса());
		Иначе
			текСтавкаНДС = СтрокаТабличнойЧасти.Номенклатура.СтавкаНДС;
		КонецЕсли;
	ИначеЕсли ОбъектДок.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС") Тогда
		текСтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
	Иначе
		текСтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСНоль();
	КонецЕсли;
	
	Если Не текСтавкаНДС = Неопределено Тогда
